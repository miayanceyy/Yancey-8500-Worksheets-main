...
numStudents <- 35
if(numStudents >= 30){
  print("Enrollment cap met")
} else {
  print("Enrollment cap not met")
}
```
...
if(length(presidents) > 5){
  print("More than 5 presidents")
} else {
  print("less than 5 presidents")
}
```
...
```{r}
gayguidesByYear <- gayguides[order(gayguides$Year, decreasing = TRUE), ]

if(gayguidesByYear$Year[1] > 1990){
  print("Yes, years after 1990")
} else {
  print("No years after 1990")
}
```
...
>  I first ordered the dataset by year using the order function from the last worksheet. Then, I got only the row with the latest year and checked only its year column. The if statement checks if this value is > 1990 and responds accordingly.
...
```{r}
classes <- c("Digital History", "VR", "AI", "Anthropology", "Tech Writing")
for(i in 1:length(classes)){
  print(paste("I am enrolled in",classes[i]))
}
```
...
```{r}
gayguidesWithBar <- gayguides[grepl("Bar", gayguides$title), ]

for(i in 1:length(gayguidesWithBar)){
  print(gayguidesWithBar$title[i])
}
...
The loop skips printing 1 and 2, because they met the <3 condition, 3 and 4 made it through the first condition and were printed, and then 5 met the last condition and broke out of the loop
...
for(i in 1:nrow(gayguides)){
  if(gayguides[i,]$city == "Greenville"){
    print(gayguides[i,]$title)
  }
}
```

*THIS WAS A PAIN PUT THIS IN THE CODING LOG*
...
for(i in 1:nrow(gayguides)){
  ...
  if(gayguides[i,]$city != "New York City"){
    next
  } 
  count = count + 1
  print(gayguides[i,]$title)
...
```{r}
historyStudents("Mia", "Digital History")
```
...
> I followed the basic formula of building a function, and then looked up "how to count number of characters in R", which led me to the nchar function. I looked over the documentation of it before use
...
> His grade is being saved in a vaiable, you would need to print the variable
...
```{r}
byStateAndYear <- function(state, year){
  for(i in 1:nrow(gayguides)){
    if(gayguides[i,]$state == state && gayguides[i,]$Year == year){
      print(paste("Found location in",state, "called", gayguides[i,]$title))
    } 
  }
}

byStateAndYear("CA", "1980")
```
...
byOccupation <- function(occupation){
  ...
  for(i in 1:nrow(BostonWomenVoters)){
    if(BostonWomenVoters[i,]$Occupation == occupation){
      rbind(results, BostonWomenVoters[i,])
    }
  }
  return(results)
}

byOccupation("Housewife")
```
*CANT FIGURE OUT HOW TO DO THIS AS A DATAFRAME*
...
countByState <- function(state){
  counter <- 0
  for(i in 1:nrow(gayguides)){
    if(gayguides[i,]$state == state ){
      counter = counter + 1
    } 
  }
  print(paste("There are",counter,"locations in",state))
}
...
```{r}
countByStateAndYear <- function(state, year){
  counter <- 0
  for(i in 1:nrow(gayguides)){
    if(gayguides[i,]$state == state && gayguides[i,]$Year == year){
      counter = counter + 1
  ...
  print(paste("There are",counter,"locations in",state,"in",year))
}

countByStateAndYear("CA", 1975)
```
...
> Loops and conditionals help to manage the logic of what a string of code does, allowing it to respond to multiple cases in scenarios when the exact state is unknown. Loops, conditionals, and functions all expand the utility of a code block, allowing them to respond to a wider array of situations. For example countByStateAndYear uses if statements to filter data, for statements to apply the action to multiple rows, and the function strucutre to apply both to multiple state/year combinations
