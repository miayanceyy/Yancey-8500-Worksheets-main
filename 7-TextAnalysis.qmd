...
> We first create a dataframe of the meta data, then add them as files to their own directory and link the metadata to the actual text data to get out working dataframe. From there we unnest and tokenize into single words, then remove standard stop words
...
> Its possible that the transition from largely written SOTU addresses to largely spoken addresses accounts for a reduced length, as it's more difficult to stand and give a full speech

(@) Filter the dataset to address this discrepancy and the recreate these statistics: 
```{r}
sotu.length.speech <- tidy_sotu %>%
...
> Count the number of occurances of each word, group by president and count the total number of words in the speech, divide those, then take the most frequent word for each president and list them
...
```{r}
sotu.fdr <- tidy_sotu %>% filter(president == "Franklin D. Roosevelt") 

sotu.fdr.tf <- sotu.fdr %>% 
  count(doc_id, word, sort = T) %>% 
  group_by(doc_id) %>% 
  mutate(n_tot = sum(n), term_freq = n/n_tot)

sotu.fdr.tfidf <- sotu.fdr %>%
  count(doc_id, word, sort = TRUE)  %>%  
  bind_tf_idf(word, doc_id, n) 
...
> A high tf-idf score tells up that a word is both heavily used and relatively unique to a specific document, which may mean that word is signifigant to that document. Watergate may have a high score in Nixons speeches because it's discussed heavily by Nixon but not mentioned in other speeches, while a word like government is used heavily, but by everyone making it less signifigant
...
Here we're looking at how language around physical health changed with the start of WW1, and if the language used encouraged people to get healthier for the war effort
``` {r}
#preprocessing
tidy_mb <- mb %>%
  #filter for 1914, start of WW1
  filter(grepl("1914",doc_id)) %>%

  #tokenize and count occurances
  unnest_tokens(word, text) %>% 
  anti_join(stop_words) %>%
  count(doc_id, word, sort = T)

#find term freqency
tidy_mb_freq <- tidy_mb %>%
  group_by(doc_id) %>%
  arrange(desc(term_freq)) %>%
  mutate(n_tot = sum(n), term_freq = n/n_tot) %>% 
  top_n(5)

#this gives us the 5 words with the highest term frequency in each month

```

We can see in this data that the words with the highest term freqency occur in september, october, and november, and include words like college, school, alchohol, physical, and children. The inclusion of these words later in the year might indicate a turn toward the physical fitness of college and school age children, and an emphasis on physical health
...
wc_full <- left_join(wc, metadata, by=c("doc_id"="filename"))
...
``` {r}
wc_sufferage <- tidy_wc %>%
  filter(word %in% c("suffrage")) %>% 
  count(doc_id, word) %>%
  arrange(desc(n))

wc_vote <- tidy_wc %>%
  filter(word %in% c("vote")) %>% 
  count(doc_id, word) %>%
  arrange(desc(n))

wc_voter <- tidy_wc %>%
  filter(word %in% c("voter")) %>% 
  count(doc_id, word) %>%
  arrange(desc(n))
 
ggplot(wc_sufferage, aes(x = doc_id, y = n )) + geom_line() + geom_point()
...
This analysis considers when discussions of suffrage and voting rose in popularity in this publication, including the frequency of the words suffrage, vote, and voter. We can see that all of these terms increased in frequency around late july to august of 1918, and steadily rise accross the limited number of issues we have, incidating an uptick in the efforts for suffrage