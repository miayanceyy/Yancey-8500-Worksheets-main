...
```{r}
data(UndergroundRR)
urr_coordinates <- undergroundRR %>% geocode(city = City, county = County, state = State, method='osm', lat = latitude, long = longitude)
head(urr_coordinates)
```
...
bwv.street <- BostonWomenVoters %>% 
  filter(Ward.Book == 6) %>%
  mutate(streetaddress = paste(Street.Number.on.April.1, Street.of.Residence.on.April.1))

bwv_coordinates <- bwv.street %>% geocode(address = streetaddress, method='osm', lat = latitude, long = longitude)
```
...
>  group, order, region, subregion, lat, long
...
ggplot() + 
  geom_map(data = usa, map = usa, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = gg_contig, mapping = aes(x = lon, y = lat), color = "red")
```
...
ggplot() + 
  geom_map(data = usa_sc, map = usa, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = gg_sc, mapping = aes(x = lon, y = lat), color = "red")
```
...
> We filter out Hawaii and Alaska in this case because they're so far geographically outside the scope of the rest of the data that they force the majority of the data to shrink and distort to include them. Removing them may shift the interpretation if those areas contain outliers or substantially different data, but in this case they don't seem to and removing them allows us to take a closer look at the rest of the data
...
```{r}
nhgis_totalpop <- nhgis_totalpop %>%
 mutate(percent_foreign = (AZF003/total_male_pop) * 100)
```
...
> The code assigns a scale of colors to the data and maps the data in those colors

Now create the map: 
```{r}
library(scales)
ggplot(data = nhgis_totalpop, aes(fill = percent_foreign)) +
  geom_sf() + scale_fill_distiller(name="Foregin Born Males", palette = "Spectral" , breaks = pretty_breaks(n = 10)) +
...
```{r}
leaflet(data=urr_coordinates) %>% addTiles() %>% addMarkers(~longitude, ~latitude, popup = paste("Location of ", urr_coordinates$First.Middle.Name, " ", urr_coordinates$Last.Name, sep=""))
```
