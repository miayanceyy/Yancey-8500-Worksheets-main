...
```{r}
data.frame(gayguides %>% select(city, state))
```

(@) What did you do to save the data to a new data frame? Why? 
I wrapped the new data in the command which creates a dataframe
>
...
```{r}
gayguides %>% filter(Year == 1975, city != "San Francisco", city != "New York City")
```
...
```{r}
gayguides %>% filter(grepl("(G)", amenityfeatures) | grepl("(L)", amenityfeatures))
```
...
BostonWomenVoters %>% mutate(birthYear = 1920 - Age)
```
...
```{r}
gayguides %>% mutate(location = paste(city,", ",state, sep=""))
```
...
```{r}
gayguides %>% mutate(titleCase = str_to_title(city))
```
...
```{r}
data(statepopulations)
statepopulations %>% select(STATE, X1850) %>% arrange(desc(X1850))
```
...
> It might indicate that that person doesn't live in a ward, doesn't have a permanent address, or wasn't living in the ward at the time of voting
...
```{r}
sum(is.na(BostonWomenVoters$Ward), is.na(BostonWomenVoters$Age))
BostonWomenVoters %>% 
  drop_na(Ward, Age)
...
```{r}
BostonWomenVoters %>% mutate(ward_missing = is.na(Ward))
```
...
```{r}
gayguides %>%
  mutate(type_factor = as.factor(type)) %>%
  count(type_factor)
...
```{r}
BostonWomenVoters %>%
  mutate(occupation_factor = as.factor(Occupation)) %>%
  levels(BostonWomenVoters$occupation_factor)
```
