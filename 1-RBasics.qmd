...
```{r}
paste("Hello","my name is","Mia")
```

(@) Can you explain what happened in 2-3 sentences? 

> The strings are printed one after the other in a single statement. Interestingly, spaces are automatically added between individual strings without adding those spaces to the strings themselves, which isn't the case in languages like python.
...
The code tests the validity of each logical statement, printing the boolean value of that statement. (Logic is checked from the outside in when there are multiple checks)
...
> The contents of each index in the vector was multiplied by 2, then printed in order
...
> This version only multiplies index 3 by 2. Index 3 holds 6, so the output is 12. (the fact that indexes start at 1 and not 0 is going to take some getting used to)

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
birthdays <- c(26,8,19,1,21)
birthdays
```
...
floor(3.5)
```
>  Returns a vectir containing the smallest integer that doesn't exceed its argument, ie it rounds down to the nearest int
...
> Row z is going row by row, getting the x and y entries for that row, and plugging them into the equation to get that row's z entry
...
> This dataset contains information from Bob Dameron's gay guides from 1965-1985. It contains 14 variables
...
> I looked at the documentation for returning the first or last parts of an object to find the tail function and what to call the second argument in head to specify number of rows.
...
> gayguides is the dataset, $ operator indicates a segment of the dataset, title indicates what column we're segmenting out, and 500 is the row. The row and column combined produce a single cell, which gets returned.
...
```{r}
gayGuidesYear <- gayguides[order(gayguides$Year),]
head(gayGuidesYear)
```
1965
...
> There are 84 observations of 28 variables. The dataset records state populations of different states over 10 year intervals.
