...
> The line of code returned all of the members of the vector except for the ones at positions 3 and 1
...
> In line 1, each member of x is being compared to its boolean statement, and x is returned containing only the members which had a true boolean statement. In line 2, every member of x is being compared to the statement x>7, and x is being returned with only the members that evaluated true to that statement
...
We created a vector y, which had the contents of vector x with the addition of names a, b, c, and d for the first 4 entries. Then, we subset vector y to show only the entry with name "d" which returns 8.35
...
> In the original line, we're working at though gayguides is a vector with only 1 dimensions, so when we subset 1:3 it means that it gives us every entry for those 2 columns. In the second line however we're treating gayguides as a matrix with 2 dimensions and specify that we want to call columns 1-3
...
housewives <- BostonWomenVoters[BostonWomenVoters$Occupation == "Housewife", ]
tail(housewives[order(housewives$Age),])
```

> I subsetted the data set down into just housewives and then ordered the filtered dataset by age and got the tail of the matrix. This let me find out the oldest women who still considered their occupation to be housewife
...
> This code gets only the city column of the gayguides matrix, then prints only the cities associated with entries 100-110

(@) Can you find all the titles of locations in South Carolina? (Hint: You'll need to use both `$` and `[]`)
```{r}
sc <- gayguides$title[gayguides$state == "SC"]
sc
```
...
```{r}
cali <- gayguides[gayguides$state=="CA", ]
cali70s <- cali[cali$Year == 1970:1979, ]
cali70s
```
...
#searches for all the occurences of (G)
containsG <- gayguides[grepl("(G)",gayguides$amenityfeatures), ]
#Filters that down further into only 1975
gin1975 <- containsG[containsG$Year == 1975, ]
...
```{r}
over50 <- BostonWomenVoters[BostonWomenVoters$Age > 50, ]
under50 <- BostonWomenVoters[BostonWomenVoters$Age < 50, ]

nrow(over50)
nrow(under50)

#2408 over 50, 8119 under 50, meaning that the age distribution of women voters is skewed young
```
...
womenOnMain <- BostonWomenVoters[grep("Main", BostonWomenVoters$Street.of.Residence.on.April.1), ]
nrow(womenOnMain)

# 4 women live on Main Street or Maine Street
```

(@) Using subsetting techniques, can you find the occupations of the five oldest women in the dataset? What were they?
```{r}
oldestWomen <- head(BostonWomenVoters[order(BostonWomenVoters$Age, decreasing = TRUE), ], 5)
occupations <- oldestWomen$Occupation
occupations

#At home, At home, [home?], Housekeeper, Widow\nRetired
```
