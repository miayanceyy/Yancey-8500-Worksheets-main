...
```{r warning=FALSE}
library(wordcloud)
topic1 <- ac.topics %>% filter(topic == 4)
...
Topic 1: Services
Topic 2: Pricing
Topic 3: Streets, parking, and cars
Topic 4: This and 5 seem very similar, calling this one public access
Topic 5: Going to call this emergency services
...
> I'm choosing the information bulletin, because I'm interested in exploring how the information in the bulletins changes over time, and what topcs are considered important enough to be covered
...
> Are the shifts in topics covered indicitave of major historical events, and more interestingly, are shifts in sentiments displayed preceeding these major events that indicate the populations mindset and the factors that might have contributed to the major events

### Step 2: Build Your Topic Model

```{r}
# Download and load your chosen corpus
download.file("https://github.com/regan008/8510-TextAnalysisData/blob/main/InfoBulletin.zip?raw=true", "InfoBulletin.zip")
unzip("InfoBulletin.zip")
# Join with metadata
metadata <- read.csv("https://raw.githubusercontent.com/regan008/8510-TextAnalysisData/main/ib-metadata.csv")
meta <- as.data.frame(metadata)

file_paths <- system.file("InfoBulletin/")
ib_texts <- readtext(paste("InfoBulletin/", "*.txt", sep=""))
ib_whole <- full_join(meta, ib_texts, by = c("filename" = "doc_id")) %>% as_tibble() 
...
tidy_ib <- tidy_ib %>% filter(!grepl('[0-9]', word), !(word %in% c("german", "germans", "germany","lk")))
...
ib.lda <- LDA(ib.dtm, k = 8, methods = "Gibbs",control = list(seed = 12345))
...
> I chose 8 topics, because that was about the point I felt like the topics started to settle into clear distinctions that I could interpret. Any more and topics seemed to start heavily overlapping with each other.
...
|--------------|-------------------|-------------|
| 1            | US government     |             |
| 2            | berlin military zone            |
| 3            | west germany/berlin             |
| 4            | military control  |             |
| 5            | police structure  |             |
| 6            | youth information |             |
...
Top 5 Words:
1. Federal, American, Governmnet, United, Information
2. military, government, omgus, berlin, information
3. berlin, united, soviet, american, government
4. military, government, page, zone, control
5. chief, page, branch, berlin, division
6. berlin, american, information, hicog, united
7. capt, maj, lt, sk, col
8. zone, ag, military, omgus, food
...
> The topics actually seemed fairly strong, although there was some heavy overlap of words like berlin, information, and bulletin
...
topics.by.year <- full_join(ib.documents, metadata, by = join_by(document == filename))

topics.by.year$issue_date <- paste(topics.by.year$month, " ", topics.by.year$year, sep = "")

topics.by.year <- topics.by.year %>% group_by(topic,year) %>% summarise(mean_val = mean(gamma))

ggplot(data=topics.by.year, aes(x=year, y=mean_val)) + geom_bar(stat="identity") + facet_wrap(~ topic, scales = "free") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
> Several of the topics saw strong spikes in specifc groups of 2-3 years following the war. In particular is makes sense that the military control and structure topics are strongest nearest the war, while the allied powers, west berlin, and youth information topics are strongest several years afterward
...
> I was addressing how the content found in these information bulletins displayed a shift in content in accordance with major historical events, and if these shifts in sentiment could be seen proceeding the events themselves. Interestingly, plotting the topics covered in the bulletin over time revealed a strong shift from mostly military content to coversations around topics like the US and Soviet Union, berlin, youth, and information. This shows an interesting shift in the culture of Berlin as a city and Germany as a whole in the years after the war, when Allied powers started exerting much stronger influence. 

Many of the topics that seemed to be discussed in the 1950-1954 years included words like united, youth, information, berlin, and soviet, which call to mind the east/west berlin split and the issues that divided the two, despite the fact that the Berlin Wall wouldn't be erected until 1961. This gives us an indication that many of these sentiments got their start much earlier than the wall itself. Additionally, the data shows a strong, relatively immediate drop off of military topics after 1947, while discussions of public police structures continued on and off for years longer.

The data in these topics does in fact show that changing public sentiments and the ideas being discussed in these bulletins can proceed major political and social changes by several years.
