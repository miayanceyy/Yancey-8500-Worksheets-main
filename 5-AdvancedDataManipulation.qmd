...
#bars and clubs by state
gayguides %>%
  select(state, type) %>%
  filter(grepl("Bars/Clubs", type)) %>%
  group_by(state) %>%
  summarize(count = n()) %>%
  arrange(desc(count))
...
gayguidesRegions <- left_join(gayguides, regions, by=c("state" = "State.Code"))
...
I did a left join joining state in gayguides with State.Code in regions, since these should match each other
...
states1970 <- gayguidesRegions %>%
  select(Year, state) %>%
  filter(Year == 1970) %>%
  group_by(state) %>%
  summarize(count = n())

states1980 <- gayguidesRegions %>%
  select(Year, state) %>%
  filter(Year == 1980) %>%
  group_by(state) %>%
  summarize(count = n())
...
gayguidesRegions %>% 
  mutate(difference = states1980 - states1970)
...
> It essentially rotates the data so rather than having sepperate columns for each year, there is one column for year, decreasing the number of coulumns and increasing the rows
...
> (Widow, Widow Retired), (Widow housekeeper, Widow Housekeeper, Widow cleaner)
...
> Cashier and clerk are among the youngest while widows at home are among the oldest, which may indicate that many women left having an occupation outside the house as they got older
...
BWV.occupations <- BostonWomenVoters %>%
    mutate(
      Occupation_clean = fct_recode(Occupation,
        ...
        "at Home" = "At Home",
        "Book-keeper" = "Bookkeeper",
        "Widow\nhousekeeper" = "Widow-Housekeeper",
        "Widow\nHousekeeper" = "Widow-Housekeeper",
        "Widow\ncleaner" = "Widow-Housekeeper"
      )
    ) %>%
    count(Occupation_clean) %>%
    arrange(desc(n))
...
```{r}
occupations <- BostonWomenVoters %>%
  mutate(
    occupation_category = fct_collapse(Occupation,
      "Domestic Work" = c("Housewife", "At Home", "Housekeeper", "At home", "Cleaner","H.W.","Married","Matron","Laundress"),
      "Education" = c("Teacher", "School Teacher"),
      "Skilled Trade" = c("Dressmaker", "Seamstress", "Milliner","Stitcher", "Waitress", "Dressmaker"),
      "Administrative" = c("Clerk", "Stenographer", "Bookkeeper","Tel. Operator", "Telephone Operator", "Secretary"),
      "Sales" = c("Saleswoman","Saleslady"),
      "Health" = c("Nurse")
    )
  ) %>%
  count(occupation_category) %>%
...
I changes clerical to administrative to include telephone operators and other types of office work, domestic may need to be split even further into homemakers and outside domestic help. If I were to split it even further I migth include subcategories for different names for the same job, like the different types of housewife
...
```{r}
dates <- c("12/01/2009","9/19/2008","04/26/2005","12/30/1979")
...
fullYears <- gayguides %>%
  select(Year) %>%
  mutate(fullYear = paste(Year,"-01-01", sep=""))
```
...
BostonWomenVoters %>%
  select(Age) %>%
  mutate(birthday = mdy(paste("07-01",1920-Age)))
```
...
```{r}
1960s <- interval(ymd("1960-01-01"), ymd("1969-12-31"))
1970s <- interval(ymd("1970-01-01"), ymd("1979-12-31"))
1980s <- interval(ymd("1980-01-01"), ymd("1989-12-31"))

datedGayGuides <- gayguides %>%
  select(Year) %>%
  mutate(date = mdy(paste("01-01",Year))) %>%
  mutate
...
createDate <- function(year){
  return(mdy(paste("01-01",year,sep="")))
}
...
> Dates might be incomplete, or might require context as to what a date in that time period might have meant. In that case, intervals like prohobition can be very helpful. It's also helpful to process dates into a uniform standard, because many handwritten recods especially write dates in different formats. lubridate allows these challenges to be fixed at a large scale and quickly, instead of sorting through it by hand.
...
How many female judges were appointed by each respective president
```{r}
data(judges)
head(judges)
...
```{r}
judges %>%
  #select the data we're working with
  select(Appointing.President..1.,Gender) %>%
  #filter for only female judges
  filter(Gender == "Female") %>%
  #group by appointing president
...
```{r}
How many of the women in the Boston Women Voters dataset lived in the same Ward, or on each street, were some streets more likely to vote than others? That might mean that women in those areas were influenced by their neighbors or social groups
...
```{r}
BostonWomenVoters %>%
  #select data we're working with
  select(Street.of.Residence.on.April.1,Ward) %>%
  #group by ward and street
  group_by(Ward,Street.of.Residence.on.April.1) %>%
  #summarize and arrange
  summarize(count = n()) %>%
  arrange(desc(count))
#we can see in the results that several streets had over 100 women with voting records, and of the highest, 5 were in Ward 8. This might indicate some social or community influence.
```
...
```{r}
yearState <- function(myYear, myState){
 return(gayguides %>%
  select(title, type, state, Year) %>%
  filter(state == myState, Year == myYear))
}
...
> Administrative positions tended to skew younger, while domestic work tended to be somewhat consistent. Domestic work was also the largest categories among many wards, with administrative the next lowest. This is likely because they hold the most occupations in their categories, but in general many women were in domestic postitions for themselves or others, with fewer in more skilled positions.
